...
public class SmartScriptParser {
	...
	private void parse(Lexer lexer) {
		...
		while (lexer.nextToken().getType() != TokenType.EOF) {
			...
			if (token.getValue().equals("{$")) {
				if (lexer.getState() != LexerState.BASIC) {
					throw new SmartScriptParserException("Wrong lexer state at current token.");
				} else {
				...
				if (token.getType() != TokenType.TEXT) {
					throw new SmartScriptParserException("Invalid token at this point.");
				}
		...
		if (stack.size() != 1) {
			throw new SmartScriptParserException("Too less end tags.");
		}
	...
	private void createTagNode(Lexer lexer) {
		...
		if (token.getType() != TokenType.Name) {
			throw new SmartScriptParserException("Expecting the name of tag, but there is no one.");
		}
	...
	private void evaluateEndTag(Lexer lexer) {
		Token token = lexer.nextToken();
		if (!token.getValue().equals("$}")) {
			throw new SmartScriptParserException("There is no end of the tag (\"$}\").");
		}
		stack.pop();
		if (stack.isEmpty()) {
			throw new SmartScriptParserException("There is more than one END-tag.");
		}
	...
	private Node createEchoTag(Lexer lexer) {
		...
		while (token.getType() != TokenType.EOF && !token.getValue().equals("$}")) {
		...
		for (int index = 0; index < size; index++) {
			elements[index] = ((Element) createElement((Token) tokens.get(index)));
		}
	...
	private Element createElement(Token token) {
		...
		case Name:
			if (token.getValue() == "=") {
				throw new SmartScriptParserException("Invalid token at this point: " + token.getValue());
			} else {
		...
		// break;
		default:
			throw new SmartScriptParserException("No miningful tag for this point. " + token.getValue());
		}
	}
	...
	private ForLoopNode createForLoopNode(Lexer lexer) {
		...
		if (token.getType() != TokenType.Name || token.getValue() == "=") {
			throw new SmartScriptParserException("Expecting variable name: " + token.getValue());
		} else {
		...
		token = lexer.nextToken();
		while (token.getType() != TokenType.EOF && iter < argc && !token.getValue().equals("$}")) {
			tokens[iter++] = token;
			token = lexer.nextToken();
		}
		...
		if (iter < ForLoopNode.minElements - 1) {
			throw new SmartScriptParserException("ForLoopNode has 3 or 4 properties not " + iter);
		} else {

			if (iter == argc && !lexer.getToken().getValue().equals("$}")) {

				throw new SmartScriptParserException("ForLoopNode has 3 or 4 properties not more.");
			}
		...
		return new ForLoopNode(variable, arguments[0], arguments[1], arguments[2]);
	}
