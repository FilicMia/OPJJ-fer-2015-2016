...
public class CreateExcelFile {
	...
	public static HSSFWorkbook createVoteResults(HttpServletRequest req,
			HttpServletResponse resp, List<Band> report)
			throws ServletException, IOException {
		HSSFWorkbook hwb = null;
		...
		try {
			...
			for (int i = 0; i < len; ++i) {
				...
				rowhead.createCell(0).setCellValue(report.get(i).getName());
				rowhead.createCell(1).setCellValue(report.get(i).getVote());
			}
	...
	public synchronized static void write(HttpServletRequest req, HttpServletResponse resp,
		...
		try {
			...
			resp.setHeader("Content-Disposition",
					"attachment;filename=powers.xls");
